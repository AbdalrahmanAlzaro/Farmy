  ...
  try {
      ...
      if (user.rows.length !== 0) {
          ...
          try {
              ...
              const insertedUserId = allData.rows[0].id; // Assuming the 'id' is generated by the database during insertion

              if (user && user.state === true) {
                  return res.status(403).json({ id: user.id, message: 'your account has been deactivated by admin' })
            
              }

              else {
                  const token = jwt.sign({
                    id: insertedUserId, name, email
                  }, secretKey);
                  return res.status(201).json({ token: token, message: 'User login successful', id: user.id });
              }
          ...
          try {
              ...
              const newUsersql = "INSERT INTO  \"user\" (username, email, password, role) VALUES ($1, $2, $3, $4) RETURNING *";
              console.log(bcryptPassword)
              // console.log(name)
              console.log(name, email, bcryptPassword)
              const newUserValues = [name, email, bcryptPassword , 'user'];


              const insertResult = await pool.query(newUsersql, newUserValues);
              console.log(insertResult,"1111111111111111111")
              const newUserData = await pool.query("SELECT * FROM  \"user\" WHERE email = $1", [email]);
              ...
              return res.status(201).json({ token: token, message: 'User login successful', id: newUserData.id });
